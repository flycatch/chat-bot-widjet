...
const Chatbox = ({ setActive }) => {
  ...
  const [arrayChat, setArrayChat] = useState([
    {
      ...
      message: ChatBotConstants.OPTIONS_REQUEST,
    },
  ]);
    ...
    if (chat !== "") {
      ...
      switch (arr.length) {
        case 4:
          arr.push({
            sender: ChatBotConstants.BOT,
            message: ChatBotConstants.USER_NAME_REQUEST,
          });
          break;
        case 6:
          arr.push({
      ...
      arr.length > 9 &&
        setApiData({
          user: {
            fullname: arr[5]?.message,
            email: arr[3]?.message,
          },
          ticket: {
            subject: arr[7]?.message,
            issue: arr[9] && arr[9]?.message,
          },
          // captcha: "rROB",
        });
      AlwaysScrollToBottom();
  ...
  const imageApi = async (ticket_id) => {
    const postData = await put(
      `${env_var.API_URL}/ticket/${ticket_id}/attach`,
      formDataConverter(fileDataObj)
    );
    return postData;
  };
  useEffect(() => {
    if (arrayChat.length === 10) {
      const postData = async () => {
        ...
        try {
          ...
          } = await post(`${env_var.API_URL}/public/tickets/create`, apiData);
          setloader(false);
          const responseMessage = {
            sender: ChatBotConstants.BOT,
            message: ChatBotConstants.TICKET_NUMBER_RESPONSE(uid,username),
          };
          setArrayChat(messages => [...messages, responseMessage]);
          ...
          const errorMessage = {
            sender: ChatBotConstants.BOT,
            message: ChatBotConstants.ERROR_RESPONSE
          }
          setArrayChat(messages => [...messages, errorMessage]);
          return err.response;
        }
      };
      postData();
